...
//CONSTRUTOR DE FORMIGA

Ant::Ant(const Instance* _instance, 
		std::vector<std::vector<double> >& _pheromoneList,
		double _alpha, double _beta) :
		solution(_instance), 
		pheromoneList(_pheromoneList), 
		instance(_instance), 
		alpha(_alpha), 
		beta(_beta) {
...
bool Ant::loop() {
	...
	//CRIA UMA MATRIZ COM O NUMERO DE MOCHILAS E O NUMERO DE ITENS
	...
	for (int k = 0; k < this->instance->getNumKnapsacks(); ++k) {
		for (int i = 0; i < this->instance->getNumItems(); ++i) {
			//VERIFICA SE O ITEM PODE SER ADICIONADO NA MOCHILA
